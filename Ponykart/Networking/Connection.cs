...
#region Pre-game Setup
        ...
        Packet CreatePacket(Message m) {
            Packet p = new Packet();
            p.CID = BitConverter.ToInt32(IDArray, 0);
            p.Contents = m.Contents;
            p.Protocol = NetworkManager.Protocol;
            p.Timestamp = System.DateTime.Now.Ticks;
            p.Type = m.Type;
            return p;
        }
