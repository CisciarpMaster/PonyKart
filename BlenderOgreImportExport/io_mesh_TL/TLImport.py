...
Group: 'Import/Export'
Tooltip: 'Import/Export OGRE mesh files'
...
This is NOT the original version. Contains some updates/bugfixes by tux3/mlkj
...
History:<br>
    * v0.6.2.1 (23-Nov-2013) - bug fixes, add suport for material inheritance and set_texture_alias, add proper texture name (tux3/mlkj)
...
import math
import os
import copy # For deep copy of materials when inheriting their parents (TUX3)
    ...
    if data!=None:
        ...
        for line in data:
            if "material" in line:
                ...
                # Find inheritances (TUX3)
                findPos = line.find(':')
                if (findPos >= 0):
                    parentName = line[findPos:].split()[1]
                    MaterialDic[MaterialName].append(':'+parentName)
                    print("Material "+MaterialName+" inherits from "+parentName);
                count = 0
            if "{" in line:
                count += 1
            if  count > 0:
        ...
        # Problem : MaterialDic is processed in alphabetical order instead of the order we filled in in
        # Have a function that process materials (= the content of the current for)
        # And while(true) for Mat in MaterialDic if it's a parent or if the parent is in allMaterials (=already processed)
        # And if it's not already processed, then process it now
        # If we reach the end of the while(true) without processing anything, stop there
        # Then for each material in MaterialDic but not in allMaterials print a warning and process them without inheriting
        
        #print(MaterialDic)
        def xCollectMaterialData_ProcessMaterial(Material):
            count = 0
            matDict = {}
            allMaterials[Material] = matDict   
            if SHOW_IMPORT_TRACE:     
                print ("Materialname: ", Material)
            # Resolve inheritance if found (TUX3)
            if (MaterialDic[Material][0][0]==':'):
                allMaterials[Material]=copy.deepcopy(allMaterials[MaterialDic[Material][0][1:]])
                matDict=allMaterials[Material]
            # Process lines
            for line in MaterialDic[Material]:
                ...
                if (count > 0) and ("texture " in line) and ('texture' not in matDict):
                    ...
                    # TUX3 : Bugfix: If imageName starts by a ", remove all quotes
                    if (imageName[0]=='"'): 
                        #print("WARNING: Texture filename contains quotes (\"), removing them");
                    ...
                    if(not os.path.isfile(file)):
                        ...
                        matDict['imageNameOnly'] = imageName
                # set_texture_alias
                if (count > 0) and ("set_texture_alias " in line) and ('texture' in matDict):
        ...
        # Try to process all materials whose parents are processable
        doneSomething = True
        while (doneSomething): # Stop when we processed everything we could
            doneSomething = False
            for Material in MaterialDic.keys():
                if (Material in allMaterials): continue
                if (MaterialDic[Material][0][0]==':'):
                    parentName=MaterialDic[Material][0][1:]
                    if parentName in allMaterials:
                        print("DEBUG: Processing "+Material)
                        xCollectMaterialData_ProcessMaterial(Material)
                        doneSomething=True
                    else:
                        print("DEBUG: Parent "+parentName+" of material "+Material+" not processed yet.");
                else:
                    print("DEBUG: Processing "+Material)
                    xCollectMaterialData_ProcessMaterial(Material)
                    doneSomething=True
                    
        # Process Materials whose parents weren't found
        for Material in MaterialDic.keys():
            if (Material in allMaterials): continue
            print("WARNING: Parent "+MaterialDic[Material][0][1:]+" of material "+Material+" not found.");
            MaterialDic[Material][0] = "" # Remove the broken inheritance
            xCollectMaterialData_ProcessMaterial(Material)
