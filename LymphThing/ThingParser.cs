...
namespace Ponykart.Stuff {
	...
	public class Parser {
		...
		private RuleInstance matchStart() {
			...
			while (true) {
				if ((tok = fetchToken(laOffset)).Type == NodeType.Tok_KeyFalse || tok.Type == NodeType.Tok_KeyModel || tok.Type == NodeType.Tok_KeyShape || tok.Type == NodeType.Tok_KeyTrue || tok.Type == NodeType.Tok_Name) {
		...
		private void lookaheadStringProperty() {
			lookaheadAnyName();
			if (laSuccess.Peek()) {
				if (fetchToken(laOffset).Type == NodeType.Tok_Assign)
					laOffset++;
