...
namespace Lymph.Core {
	...
	public class Spawner {
		...
		public Thing Spawn(ThingEnum type, ThingTemplate template) {
			...
			Thing actor;
			// this just keeps the whole colours thing out of the parameters for now
			var colours = new AntigenColour(AntigenColour._red);

			switch (type) {
				case ThingEnum.Antibody:
					actor = new Antibody(template, colours);
					Invoke(OnAntibodyCreation, actor);
					break;
				case ThingEnum.GenericEnemy:
					actor = new GenericEnemy(template, colours);
