...
namespace Lymph.Core {
	public class PlayerCamera : IDisposable {
		...
		public PlayerCamera() {
			...
			Camera.Position = new Vector3(0f, Constants.CAMERA_DISTANCE, 0f);
			//have to offset a bit otherwise it won't work due to gimbal lock
			Camera.LookAt(new Vector3(0, 0, -0.00001f));
