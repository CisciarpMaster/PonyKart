...
namespace Lymph.Handlers {
	...
	public class LevelUIHandler {
		...
		void MakeLevelUI() {
			...
			commandsLabel = new Label("commands label") {
				...
				Text =
					...
					//"[Num 1-5] Change color schemes\r\n" +
					"[M] Turn music on or off (requires level reload)\r\n" +
					"[P] Turn sounds on or off\r\n" +
					"[N] Play music now\r\n" +
					//"[F2] Add acceleration force to player\r\n" +
					//"[F] Add large force to player\r\n" +
					"[T] Turn trippy mode on or off (not retroactive)\r\n" +
		...
		void MakeMainMenuUI() {
			...
			level1Button.MouseClick += (o, e) => LKernel.Get<LevelManager>().LoadLevel("Level1");
			...
			level2Button.MouseClick += (o, e) => LKernel.Get<LevelManager>().LoadLevel("Level1");
			...
			level3Button.MouseClick += (o, e) => LKernel.Get<LevelManager>().LoadLevel("LargeFlat");
			...
			level4Button.MouseClick += (o, e) => LKernel.Get<LevelManager>().LoadLevel("Vessel");
			...
			mainMenuControls.Add(level5Button);
			level5Button.MouseClick += (o, e) => LKernel.Get<LevelManager>().LoadLevel("Fat");

			level6Button = new Button("Level 6") {
				Location = new Point((int)(Constants.WINDOW_WIDTH / 2) - 100, 300), // the 100 is half of 200, which makes sure the button is centered
				Size = new Size(200, 40),
				Skin = UIResources.Skins["ButtonSkin"],
				Text = "Level 6",
				TextStyle = {
			...
			mainMenuControls.Add(level6Button);
			level6Button.MouseClick += (o, e) => LKernel.Get<LevelManager>().LoadLevel("shittyterrain");

			quitButton = new Button("Quit") {
				Location = new Point((int)(Constants.WINDOW_WIDTH / 2) - 100, 350), // the 100 is half of 200, which makes sure the button is centered
				Size = new Size(200, 40),
				Skin = UIResources.Skins["ButtonSkin"],
				Text = "Quit",
				TextStyle = {
